...
void ConclusionJson(std::ostream& output, OutRequests& out_requests, std::vector<DirectoryRequest>& directory_requests, MapSettings& map_settings, std::map<std::string_view, Coordinates>& stops_on_buses, std::vector<Bus*>& buses)
{
	...
	for (size_t i = 0; i < out_requests.size(); ++i) {
		...
		if (directory_requests[i].type == "Bus") {
			...
			output << std::endl;
			/*std::cout << "{" << std::endl;
			std::cout << "\"curvature\": " << bus->distant_bus.second << "," << std::endl;
			std::cout << "\"request_id\" : " << directory_requests[i].id << "," << std::endl;
			std::cout << "\"route_length\" : " << bus->distant_bus.first << "," << std::endl;
			std::cout << "\"stop_count\" : " << (*bus).stops.size() << "," << std::endl;
			std::cout << "\"unique_stop_count\" : " << uniqueStops.size() << std::endl;
			std::cout << "}" << std::endl;*/
		}
