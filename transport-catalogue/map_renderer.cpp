...
void DrawSvg(std::ostream& output, MapSettings& map_settings, std::map<std::string_view, Coordinates>& stops_on_buses, std::vector<Bus*>& buses)
{
    ...
    size_t i = 0;
    SphereProjector proj(stops_on_buses.begin(), stops_on_buses.end(), map_settings.width_,map_settings.height_,map_settings.padding_);
    ...
    for (const auto& bus : buses) {
        ...
        //!!!!!!!Вставить правильные тесты и проверить верность данного участка!!!!!!!!!!!!!
